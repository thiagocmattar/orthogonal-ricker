...
\tightsection{Is This a GELU Problem? Completed ReLU Full-pass Result}

The site-scope experiments raised a sharper hypothesis: perhaps the GELU MLP nonlinearity itself makes high MLP activation sparsity expensive. We therefore changed the Pythia MLP activation to ReLU while keeping the one-pass MiniPile recipe fixed. The completed ReLU set now contains AdamW, RN \(w=1,c=0.05,\sigma=0.05\), OR \(w=1,c=0.05,\sigma=0.05\), L1N \(w=5\), and OL1 \(w=5\).

\begin{table}[H]
\centering
\caption{GELU versus ReLU endpoint comparison for completed full-pass runs. Validation loss and MLP exact-zero/near-zero endpoint summaries are exact values from each run's training \code{metrics.json}.}
\small
...
GELU & L1N & 5.2144 & +0.3826 & 0.00\pct & 81.05\pct & Highest GELU near-zero mass, still high loss. \\
GELU & OL1 & 5.2342 & +0.4024 & 0.00\pct & 73.39\pct & Orthogonal L1 does not rescue GELU loss. \\
...
ReLU & L1N & 4.7967 & -0.0437 & 95.35\pct & 95.75\pct & Sparse ReLU regime persists under L1 pressure. \\
ReLU & OL1 & 4.7981 & -0.0423 & 93.02\pct & 93.63\pct & Orthogonal L1 stays in the sparse low-loss regime. \\
\bottomrule
\end{tabularx}
\end{table}
...
OL1 & \nzvec{83.5}{96.6}{96.8}{96.4}{96.2}{95.5} & \nzvec{39.6}{19.4}{14.6}{15.8}{15.2}{12.7} & \nzvec{49.1}{24.9}{24.7}{33.7}{29.9}{21.0} \\
L1N & \nzvec{94.1}{97.9}{97.1}{96.5}{96.1}{95.1} & \nzvec{38.6}{15.4}{11.7}{11.6}{12.8}{13.5} & \nzvec{21.7}{21.0}{25.6}{24.5}{29.6}{18.6} \\
\bottomrule
\end{longtable}

\textbf{Current interpretation.} This is the most interesting result in the update. Across RN, OR, L1N, and OL1, ReLU moves the model to roughly 93--96\pct{} exact-zero MLP hidden activation mass with no observed validation-loss penalty, and with point estimates slightly below the ReLU AdamW baseline. That is qualitatively different from the corresponding GELU pressure runs, where high near-zero MLP mass paid about 0.38--0.41 validation-loss points.

\textbf{Caveat.} These are still one-seed runs. The ReLU story should be treated as a strong follow-up target, not as a final paper claim, until the key ReLU candidates are repeated across seeds.
...
\caption{Completed ReLU learning curves for AdamW, RN, OR, L1N, and OL1.}
\end{figure}
\clearpage
...
\caption{Completed ReLU post-hoc clipping frontiers in one comparable view. The MLP panel is the key activation-function result: RN, OR, L1N, and OL1 start near the high exact-zero regime with AdamW-level validation loss, unlike the GELU high-pressure tradeoff.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.88\textheight,keepaspectratio]{../../figures/73-status-update-relu-coupling-density-comparison.pdf}
\caption{Completed ReLU coupling-density diagnostic. Rows compare AdamW, RN, OR, L1N, and OL1; columns add the post-LayerNorm MLP input density before the MLP-hidden, residual-stream, and attention-output densities.}
\end{figure}
\clearpage

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.88\textheight,keepaspectratio]{../../figures/71-status-update-relu-pressure-weight-diagnostic.pdf}
\caption{Completed ReLU pressure-weight diagnostic for AdamW, RN, OR, L1N, and OL1. Columns aggregate final-checkpoint weights across all six layers for MLP-hidden input weights, residual-stream output-projection weights, and attention QKV weights.}
\end{figure}
\clearpage
...
\begin{itemize}
  ...
  \item ReLU changes the regime: completed ReLU RN/OR/L1N/OL1 runs reach roughly 93--96\pct{} exact-zero MLP hidden activations with AdamW-level validation loss.
  \item The ReLU result is the priority follow-up; the next step is repeating key runs across seeds.
\end{itemize}
