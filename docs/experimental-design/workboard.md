...
| `DIAG-05` | resolved | A2 post-hoc clipping evidence | Add one numbered, config-driven six-source clipping diagnostic that reuses the existing operator/evaluator, runs the reviewed grid sequentially under one lifecycle and lock, and validates every source/cutoff row plus exact logical-product arithmetic | Reviewed design `3a4b047`; config `020`, guarded definitive and non-evidence calibration commands, one sequential lifecycle/lock, exact source/cache/checkpoint/diagnostic-019 pins, 30-row completeness, and integer `R_model` validation are implemented. Final verification passed 276 focused tests and the full suite at 609 passed / 3 skipped. Definitive execution remains separately approval-gated. |
