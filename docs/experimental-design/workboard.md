...
| `DESIGN-04` | in_progress | A1 three-cell high-LR extension | Review the exact eight-cell A1 design while preserving configs/runs `001`–`005` and allocating only `1.6e-2`, `3.2e-2`, and `6.4e-2` afterward | User direction on 2026-08-26; exact design SHA to be recorded in the reviewed manifest before config allocation |
...
| `OPS-12` | in_progress | A1 three-cell high-LR launch | Bind a new definitive exception to exact configs `001`–`008`, require completed reuse of `001`–`005`, and admit only pending `006`–`008` on three distinct homogeneous A40 GPUs under one coordinator and lock | Exact runner authorization, failure-drain and reuse tests, clean committed launch recipe, same-hardware ETC, and bounded RunPod envelope |
...
| --- | --- | --- |
| A1 | ready | Five cells are eligible, valid evidence; run only the directed `1.6e-2`, `3.2e-2`, and `6.4e-2` cells before applying the eight-cell rule |
