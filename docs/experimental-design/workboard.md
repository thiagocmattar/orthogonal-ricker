...
| `DIAG-02` | not_applicable | A2 pretraining | Add pooled-RMS schema fields or new training-time spillover machinery | User direction on 2026-08-27 and the existing-artifact audit: schema v3 already stores layer counts, RMS, threshold hits, bins, underflow/overflow, source identity, and coverage. `diagnostics.md` defines the derived reductions. One common histogram recipe is pinned later in the post-hoc diagnostic config; no implementation or pretraining blocker remains. |
...
| `OPS-16` | resolved | A2 definitive launch preparation | Bind exact configs `012`-`017` to two distinct homogeneous A40 workers under one coordinator and lock, using production-shaped A2 timing rather than the provisional ETC | Exact runner authorization; accepted concurrent 600-second control/L1 calibration; independently verified retrieval SHA-256 `25b11ccf6080e22ccf258e4751807cd81905f5b8304f4993d15f73893b0d25d5`; 75 focused runner tests and full suite 542 passed / 3 skipped; strict check at 0 errors/warnings; infrastructure smoke `028-20260827-142026-d9e5dfed`; definitive billable launch remains separately approval-gated |
...
| A2 | ready | Exact configs `012`–`017` are materialized and calibration/ETC is accepted; definitive launch remains separately approval-gated. |
