...
| `DIAG-04` | resolved | A2 result handoff | Measure observed `R_block` and `R_model` for every accepted A2 pressure/control checkpoint used in the result table | Config `019-a2-activation-propagation`, run `001-20260828-110533-6ac813e6`, completed from clean launch Git `96621bcb73f74933f95b8b5fcd9a63ec2e15e3ff` over all 152 selection sequences and 311,296 tokens; schema-v5 artifact SHA-256 `709599e0e68abe8350a720e6a37f392f19aadaf42fb681d28308b62db44cf3d9`. Observed logical-opportunity evidence must be reused in later accepted pressure/intervention comparisons rather than recomputed from activation marginals. |
| `DIAG-05` | resolved | A2 post-hoc clipping evidence | Add one numbered, config-driven six-source clipping diagnostic that reuses the existing operator/evaluator, runs the reviewed grid sequentially under one lifecycle and lock, and validates every source/cutoff row plus exact logical-product arithmetic | Reviewed design `3a4b047`; config `020`, guarded definitive and non-evidence calibration commands, one sequential lifecycle/lock, exact source/cache/checkpoint/diagnostic-019 pins, 30-row completeness, and integer `R_model` validation are implemented. Final verification passed 276 focused tests and the full suite at 609 passed / 3 skipped. At implementation commit `904bdc4`, the outcome-suppressed full-partition calibration measured 2.167 s setup, 4.075 s evaluation, 76,395 tokens/s, and 4,888 MiB peak reserved memory on the local RTX 5070 Ti; no raw attempt was created. Definitive execution remains separately approval-gated. |
...
| `OPS-15` | in_progress | B1+ config allocation | Add exact physical-cell materialization contracts for later non-A2 groups | A3 scope is closed by commit `8c130c3`: 80 focused passes cover its exact field/grid/decision contract, fingerprints, duplicates, prior A1/A2 evidence preservation, and later-group fail-closed behavior. B/C group contracts remain unimplemented and blocked. |
...
| `DESIGN-07` | resolved | A2 post-hoc clipping evidence | Review the exact six-source, joint `[a, m, h, q_post, k_post, v]`, five-cutoff clipping design and its descriptive claim boundary | Explicit user approval on 2026-08-28 of design commit `3a4b047b1f4712d07b32314461913aae09cc46a7`; reviewed manifest scope `[A2-relu-control, A2-l1-screen]` |
...
| `PLOT-01` | in_progress | paper release | Implement the declared figure families from pinned artifacts | A1 outputs are accepted; the replacement A2 spillover plane, layerwise atlas, and pooled-site atlas have exact reductions and focused tests, with rendered visual/adversarial acceptance pending. |
| `PLOT-02` | in_progress | A2 post-hoc clipping figure | Render the exact 30-point A2 quality–`R_model` cohort as one two-panel figure with absolute and within-checkpoint views, plus a complete companion table and provenance | Fail-closed renderer, deterministic reduction/export tests, and synthetic publication-profile QA are implemented. Acceptance remains blocked on definitive diagnostic `020`, exact evidence pinning, and visual/adversarial review of the real-data render. |
...
| A2 | complete | Configs `012`–`017` and diagnostics `018`–`019` completed and provide the reviewed seed-0 loss, spillover, and observed `R_block`/`R_model` evidence. Supplemental diagnostic `020` and its figure remain separately tracked by `DIAG-05`/`PLOT-02` and do not authorize or reopen A2 pretraining. |
