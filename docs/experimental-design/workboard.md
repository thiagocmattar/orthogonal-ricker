...
| `OPS-04` | in_progress | A1+/C1+ | Define the throughput-optimal physical microbatch and gradient-accumulation decomposition for each model on its pinned RunPod GPU class while preserving 128 sequences per optimizer update | Commits `0852a2e`, `037a705`, `131a142`; 55 profiler tests; all three live idle-GPU artifacts passed on 2026-08-25 and proposals are recorded in `protocol.md`; explicit value review/freeze still required |
...
`CLOUD-01`, `OPS-04`, `OPS-05`, and `OPS-06` are necessary infrastructure
evidence for any future concurrent RunPod launch, but their resolution is not
authorization. `AGENTS.md` currently requires serial definitive execution.
Concurrency requires an explicit repository-policy revision and a compatible
reviewed plan; until then, only the infrastructure smoke may use worker slots.
