...
| `DIAG-04` | in_progress | A2 result handoff | Measure observed `R_block` and `R_model` for every accepted A2 pressure/control checkpoint used in the result table | Immutable config `019-a2-activation-propagation` pins runs `012`-`017` and the complete selection partition; execution and terminal artifact remain pending. Observed logical-opportunity evidence must be reused in later accepted pressure/intervention comparisons rather than recomputed from activation marginals. |
...
| A2 | analysis in progress | Configs `012`–`017` and diagnostic `018` completed and provide the reviewed seed-0 loss and spillover evidence. Diagnostic `019` is materialized but not run; observed `R_block`/`R_model` remain pending for the required result handoff. |
