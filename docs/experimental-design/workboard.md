...
| `DIAG-05` | resolved | A2 post-hoc clipping evidence | Add one numbered, config-driven six-source clipping diagnostic that reuses the existing operator/evaluator, runs the reviewed grid sequentially under one lifecycle and lock, and validates every source/cutoff row plus exact logical-product arithmetic | Reviewed design `3a4b047`; config `020`, run `001-20260828-130123-cefae393`, completed from clean launch Git `5f1f5a7aa079d46c4d2855ee7c2a16d027abe37d` over six sources, five thresholds, 152 selection sequences and 311,296 tokens per point. The 30-row `clipping_frontier.jsonl` SHA-256 is `9322c54c61fa68e5edb5b191d417a77e5c985369d2a10ed66b5cc05676322f23`; strict integrity passed with zero errors/warnings. |
...
| `PLOT-02` | resolved | A2 post-hoc clipping figure | Render the exact 30-point A2 quality–`R_model` cohort as one two-panel figure with absolute and within-checkpoint views, plus a complete companion table and provenance | Diagnostic `020` run `001-20260828-130123-cefae393` and artifact SHA-256 `9322c54c61fa68e5edb5b191d417a77e5c985369d2a10ed66b5cc05676322f23` are pinned. The complete-grid main axes, `t <= 0.10` detail insets, grouped redundant legends, caption, 30-row table, and provenance passed deterministic tests, publication-profile checks, independent adversarial review, PNG inspection, and PDF re-render inspection. |
...
| A2 | complete | Configs `012`–`017` and diagnostics `018`–`020` completed and provide the reviewed seed-0 loss, spillover, observed `R_block`/`R_model`, and post-hoc clipping-frontier evidence. The figure remains separately tracked by `PLOT-02`; none of this authorizes or reopens A2 pretraining. |
