...
| `DESIGN-02` | in_progress | A1 boundary extension | Review the exact four-cell A1 design at a committed SHA while preserving configs/runs `001`–`003` and allocating only the proposed `4e-3` cell afterward | Exact-SHA approval of `a1-boundary-extension-review-packet.md`, followed by a reviewed manifest for `[A1-lr-screen]` |
...
| --- | --- | --- |
| A1 | blocked | The three original configs remain eligible, valid evidence at execution commit `276da7cd8e9142da48b95e12b46a99d61367ca8f`; exact-SHA review and one pending `4e-3` cell are required before applying the four-cell rule and freezing `lr_14m` |
