...
| `OPS-04` | in_progress | A1+/C1+ | Define the throughput-optimal physical microbatch and gradient-accumulation decomposition for each model on its pinned RunPod GPU class while preserving 128 sequences per optimizer update | Commits `0852a2e`, `037a705`, `131a142`; 55 profiler tests; reviewed values and live idle-GPU artifacts for all three models still required |
| `CLOUD-01` | in_progress | cloud smoke/launch | Set up RunPod for agent operation from the official `agent-setup.md`: install the skill bundle and Codex marketplace plugin, complete user OAuth, and define secure storage, environment, provenance, and teardown practice | Installed bundle/marketplace and authenticated MCP; commits `8fc134b`, `82f67d3`; pinned `runpodctl` v2.8.0 binary verified locally; [operator procedure](../runbook.md#runpod-operations); local CLI authentication, combined live SSH/GPU proof, artifact retrieval, and teardown still required; no secret committed or printed |
| `OPS-05` | in_progress | concurrent launch | Implement bounded concurrent execution of distinct immutable configs under one authoritative one-host case-runner coordinator; never start multiple case runners | Commits `2e7e27f`, `64690ed`, `20e3efb`, `e28e8a4`, `38814f8`, `6af916c`, `86d878a`, `33bef84`; full local suite passed; live one-host GPU proof still required; multi-Pod mode is unsupported |
| `OPS-06` | in_progress | concurrent launch | Validate the complete concurrent RunPod workflow with infrastructure-only smoke tests before any scientific run | Commit `33bef84`; full local suite: 439 passed, with 3 Windows link-capability tests deferred to Linux; two-worker RunPod proof and clean teardown still required |
...
A concurrent RunPod launch additionally requires `CLOUD-01`, `OPS-04`,
`OPS-05`, and `OPS-06` to be resolved. Until then, the serial case-runner
contract remains the only permitted definitive launch path.
