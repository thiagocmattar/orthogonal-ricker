...
| `DESIGN-06` | in_progress | A2 screen | Refine A2 to one seed-0 ReLU control plus h-only L1 at lambda `{0.1, 1, 5}`; do not add seeds for the full response | Resolve `OPS-08` without changing the frozen scientific path; then exact A2 packet and user approval at one committed design SHA |
...
| A2 | blocked | Resolve `OPS-08` so it admits only the four proposed seed-0 cells while the scientific path stays frozen; then review exactly `[A2-relu-control, A2-l1-screen]` at the resulting commit. |
