...
| `DIAG-05` | open | A2 post-hoc clipping evidence | Add one numbered, config-driven six-source clipping diagnostic that reuses the existing operator/evaluator, runs the reviewed grid sequentially under one lifecycle and lock, and validates every source/cutoff row plus exact logical-product arithmetic | Review `a2-clipping-review-packet.md`; then focused config, lifecycle, source-identity, completeness, and `R_model` count tests before calibration or execution |
...
| `PLOT-02` | open | A2 post-hoc clipping figure | Render the exact 30-point A2 quality–`R_model` cohort as one two-panel figure with absolute and within-checkpoint views, plus a complete companion table and provenance | Reviewed design; accepted diagnostic `020`; deterministic reduction/export tests; visual and adversarial review |
...
| A2 | complete | Configs `012`–`017` and diagnostics `018`–`019` completed and provide the reviewed seed-0 loss, spillover, and observed `R_block`/`R_model` evidence. |
